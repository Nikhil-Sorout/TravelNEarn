...
const PublicSearchScreen = ({ navigation, route }) => {
  ...
  const fetchCoordinates = async (origin, destination) => {
    try {
      ...
      const response = await fetch(
        `${baseurl}map/getdistanceandcoordinate?origin=${encodeURIComponent(origin)}&destination=${encodeURIComponent(destination)}`
      );
      const data = await response.json();
  ...
  useEffect(() => {
    if (coordinates.length > 0 && originCoords && destinationCoords && !mapRegion) {
      fitMapToRoute(coordinates, originCoords, destinationCoords);
    }

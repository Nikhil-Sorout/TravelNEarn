...
const PublishTravelDropLocation = ({ navigation, route }) => {
    ...
    if (search && search.trim()) {
      try {
        ...
        if (reverseResponse.data?.results?.length > 0) {
          ...
          setFullGoing(search);
          setGoing(formatDisplayText(search));
        }
      } catch (error) {
        console.error("Error handling search parameter:", error);
        // Fallback to original search parameter if any step fails
        setFullGoing(search);
        setGoing(formatDisplayText(search));
      }
    }
  ...
  const handleQuickAddressSelect = (addressType) => {
    ...
    if (!address) {
      ...
      navigation.navigate("AddressBook", {
        isSelecting: true,
        returnScreen: "PublishTravelDropLocation",
      });
      return;
    }
  ...
  // Initialize with default location from user's destination
  useEffect(() => {
    const initializeDefaultLocation = async () => {
      try {
        // Get the destination from AsyncStorage (set by user in previous screen)
        const goingLocation = await AsyncStorage.getItem("goingLocation");
        
        if (goingLocation && goingLocation.trim()) {
          // Use the user's destination as default
          setFullGoing(goingLocation);
          setGoing(formatDisplayText(goingLocation));
          
          // Fetch coordinates for the destination
          await handleSearchParameter(goingLocation);
        } else {
          // Fallback to India center if no destination set
          setLocation({
            latitude: 28.6139,
            longitude: 77.2090,
            latitudeDelta: 0.01,
            ...
            <TouchableOpacity
              onPress={async () => {
                try {
                  ...
                  navigation.navigate("PublicSearchScreen");
                } catch (error) {
...
export default memo(PublishTravelDropLocation); 